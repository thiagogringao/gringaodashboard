...
cp -r "$BACKUP_DIR/backend/cache" "$APP_DIR/backend/" 2\u003e/dev/null || echo "Cache antigo não restaurado (não encontrado)."
cp "$BACKUP_DIR/backend/auth.db" "$APP_DIR/backend/" 2\u003e/dev/null || echo "auth.db antigo não restaurado (não encontrado)."
